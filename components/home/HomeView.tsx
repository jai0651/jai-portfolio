...
/*
 * Presentational only. This used to fetch its own data in useEffect, which
 * meant the server rendered a skeleton and the homepage shipped zero words of
 * text to any crawler that does not execute JavaScript. The data now arrives
 * as props from a server component and the same markup is in the initial HTML.
 */
export default function HomeView({ profile, skills, resumeUrl }: HomeData) {
  return (
    <>
      {/* ── Hero ─────────────────────────────────────────────────── */}
